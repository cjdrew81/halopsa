...
function TreeSizeHtml { 
    ...
    for ($i=0;$i -lt $pathsArray.Length; $i++){
        ...
        sbAppend "<h1 class='display-4'><img alt='Embedded Image' src='@Base64EncodedImage@'</img></h1>"
        $machine = hostname
        sbAppend "<h2>Disk utilization report for ($($dirInfo.FullName)) on $($machine)</h2>"
        sbAppend "<p class='lead'>Generated for %ClientName% at %LocationName% on @CurrentDateTime@</p>"
        sbAppend "</div>"
        sbAppend "</div>"
        ...
        {
            sbAppend "<div class='alert alert-info'>After a depth of $folderSizeFilterDepthThreshold folders, branches with a total size less than $folderSizeFilterMinSize bytes are excluded</div>"
        }    
...
TreeSizeHtml -paths "@paths@" -reportOutputFolder "%tempdir%" -htmlOutputFilenames "@FileName@" -folderSizeFilterDepthThreshold @folderSizeFilterDepthThreshold@ -folderSizeFilterMinSize @folderSizeFilterMinSize@ -topFilesCountPerFolder @topFilesCountPerFolder@ -displayUnits @displayunits@ -zipOutputFilename "@ZipFileName@"
